        ...
        for m in self.model:
            ...
            if m.f==-4:
                ...
                if len(list(m.named_parameters())) > 0:
                    ...
                    if isinstance(m, (FDFTM_cross, FDFTM_ourv2_cross, fusion_two, fusion_twov1, fusion_twov2)):#  [rgb, ir]
                        
                        x3 = m([rgb, ir])
                    else:# concat 后的 tensor
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in ((
            ...
            LDConv, MANet, MANet_FasterBlock, MANet_FasterCGLU, SFEM, SFEM_new,
            ...
            if m in ((
                ...
                CSPStage, SPDConv, RepBlock, CSP_EDLAN, RGCSPELAN, CSP_PTB, SFEM, MANet,SFEM_new,
        ...
        ### 2026-06 ###
        # elif m in (fusion_two, fusion_twov2):
        #     c1 = make_divisible(min(args[0], max_channels) * width, 8)
        #     c2 = c1 * 2
        #     args = [c1]


        elif m in (FDFTM_cross, FDFTM_ourv2_cross, fusion_two, fusion_twov1 ,fusion_twov2):# 2026/6/17 zzh 修改
            c1 = make_divisible(min(args[0], max_channels) * width, 8)  # 单路真实通道
            c2 = c1 * 2                                                # 输出 concat 后通道
            args = [c1]

      
        # elif m in (FDFTM_cross,FDFTM_ourv2_cross):  # # 2026/6/17 zzh 修改
        #     c2 = ch[f]
        #     args = [c2, *args[1:]]
        elif m in (FDFTM_ourv1,FDFTM_ourv2,FDFTM):  # 2026/6/15 zzh 修改
            c2 = ch[f[0]]
            args = [c2, *args[1:]]       
        elif m is CIFusion:
            args = [args[0]]
