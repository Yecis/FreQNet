...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 
           ...
           'C3k2_DynamicFilter', 'C3k2_TSSA', 'A2C2f_CGLU', 'A2C2f_DFFN', 'HAFB', 'A2C2f_FRFN', 'C3k2_MobileMamba', 'C3k2_MambaOut', 
            'LoGStem', 'C3k2_LFEM', 'PST','SFEM_new'
           ]
...
class DWT_IDWT(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.dwt = DWTForward(J=1, wave='haar')
        self.idwt = DWTInverse(wave='haar')

        self.ll_conv = Conv(channels, channels, 3)
        self.lh_conv = Conv(channels, channels, 3)
        self.hl_conv = Conv(channels, channels, 3)
        self.hh_conv = Conv(channels, channels, 3)
    ...
    def forward(self, x):
        ...
        high = torch.stack([lh, hl, hh], dim=2)
...
class EdgeConv(nn.Module):
    def __init__(self, channels):
        ...
        weight = torch.tensor(kernel).view(1, 1, 3, 3)
        ...
        self.refine = Conv(channels, channels, 3)

    def forward(self, x):
        return x + self.refine(self.conv(x))


class SFEMBlock(nn.Module):
    def __init__(self, c):
        super().__init__()
    ...
    def forward(self, x):
        ...
        return self.fuse(torch.cat([a, b, c, d], dim=1)) + x
